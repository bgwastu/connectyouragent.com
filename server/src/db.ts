...
export function getSession(code: string) {
  return db
    .query<{ code: string; status: string; agent_os: string | null; agent_arch: string | null; agent_host: string | null; created_at: string; closed_at: string | null }, [string]>(
...
export function setAgentMeta(code: string, os: string, arch: string, host: string, user: string) {
  db.prepare("UPDATE sessions SET agent_os = ?, agent_arch = ?, agent_host = ?, agent_user = ?, updated_at = datetime('now') WHERE code = ?").run(os, arch, host, user, code);
}
